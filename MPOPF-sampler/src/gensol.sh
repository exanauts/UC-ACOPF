...
# Study guide:
#    https://shellscript.sh/tips/getopt
#    https://linuxize.com/post/bash-case-statement
#    https://cyberciti.biz/faq/bash-for-loop
...
set -- $options
for i  # Infinite loop. -- is passed after all arguments, which calls break.
do
    case "$i" in
	-w)
        W=$2
...
#                            result="${nlp}_case${C[$c]}_t${t}_h${h}_${sc}_ls${ls}_rs${RS[$c]}_cl${cutline}_cg${cutgen}_pt${perturb}_${WSTR}_${PSTR}_${QSTR}_${LA}"
                        ...
                        result="${nlp}_case-${C[$c]}_t-${t}_h-${h}_sc-${sc}_ls-${ls}_rs-${RS[$c]}_cl-${cutline}_cg-${cutgen}_pt-${perturb}_pf-${pfsolve}_wsrt-${WSTR}_pstr-${PSTR}_qstr-${QSTR}_la-${LA}_mlim-${ML}"
