...
function get_mpcpfmodel(circuit, demand)
    # mpcpf is only used in solve_cold.
    # This only solves the powerflow equation, to satisfy the constraints without objective value
    m = Model()
...
function get_mpcobjectivevalue(m, circuit, T)
    ...
    Args:
        ...
        circuit (Circuit) - corresponding circuit for the model
        T (int) - end time to evaluate

    Returns:
        value of objective function for 1:num_gens, 1:T
    =#
    baseMVA = circuit.baseMVA
    gen = circuit.gen
